...
export default function ResultsSection({
  ...
  const skeletonCount = data.requestedCount ?? 6;
  const shortfall =
    !isLoading &&
    data.requestedCount !== undefined &&
    data.returnedCount !== undefined &&
    data.returnedCount < data.requestedCount;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="font-display text-3xl text-white tracking-wide">
            {isLoading
              ? "SCANNING FOR MOMENTS..."
              : `${data.clips.length} VIRAL CLIPS FOUND`}
          </h2>

          {/* Shortfall warning */}
          {shortfall && (
            <p className="text-xs text-yellow-500/80 mt-1">
              Transcript only had enough content for {data.returnedCount} strong clips out of the {data.requestedCount} you requested.
            </p>
          )}

          {/* Estimated timestamps notice */}
          {!isLoading && data.usedEstimatedTimestamps && (
            <p className="text-xs text-zinc-500 mt-1">
              ⚠️ Timestamps are estimated — no timestamps detected in source
            </p>
          )}
        </div>
